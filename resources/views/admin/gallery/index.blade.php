...
@section('content')
    <div class="flex items-center justify-between">
        <div class="text-sm text-white/60">Success Gallery</div>
        <a
            ...
            <tbody class="divide-y divide-white/10">
                @forelse ($items as $item)
                    <tr class="hover:bg-white/5">
                        <td class="px-4 py-3">
                            <div class="font-medium text-white">{{ $item->title ?: 'Untitled' }}</div>
                            @if ($item->caption)
                                <div class="mt-1 text-xs text-white/50 line-clamp-2">{{ $item->caption }}</div>
                            @endif
                            @if ($item->is_published && $item->images_count === 0)
                                <div class="mt-2 text-xs text-amber-200">Published but has no images — it will not show on the site.</div>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-white/70">{{ $item->images_count }}</td>
                        <td class="px-4 py-3 text-white/70">{{ $item->is_published ? 'Yes' : 'No' }}</td>
                        <td class="px-4 py-3">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-10 text-center text-white/60">
                            No gallery cards yet. Click “Add card”.
                        </td>
                    </tr>
                @endforelse
            </tbody>
